...
if duplicates:
    ...
    for dup in duplicates:
        ...
        if post_data.get("premium"):
            ...
            continue
        # URL重複のみ削除（タイトル類似は削除しない）
        if dup.get("type") != "URL重複":
            continue
        print(f"🗑️ URL重複を削除: {slug[:55]}")
...
forbidden_score = ["かもしれない","とみられる","とされる","示唆している","と見られる","試される局面だ","深刻な局面だ","どこへ向かうのか","避けられない","この判断は妥当だ","妥当だと考えられる"]
low_quality = []
