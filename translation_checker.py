...
for i, post in enumerate(posts):
    ...
    prompt = (
        ...
        f"2. Critical role errors (Senate/House confusion)\n"
        f"3. pleads guilty mistranslated as 起訴\n"
        f"4. English in brackets like [Name] still remaining\n"
        f"5. Any Cyrillic or Hangul characters mixed in\n\n"
        f"If you find NEW mistranslation patterns not obvious from context, suggest:\n"
        f"SUGGEST: [wrong katakana] -> [correct katakana] | reason\n\n"
        f"Format: FIX: [wrong] -> [correct] | reason\n"
        f"Or: SUGGEST: [pattern] -> [correct] | reason\n"
