...
def screen_article(title, summary="", recent_titles=None):
    ...
    prompt = (
        ...
        "1. Is this newsworthy? Answer yes if: war/conflict, diplomacy/summit, economic news, politics, business/corporate news, crime, social issues, sports, science/tech, environment. Answer no if: clearly trivial, opinion pieces, roundups/summaries of already-known events, 'what to watch' preview articles, political cartoons/satire compilations, weekly photo galleries, 'best of' collections, news quizzes, photo essays, 'in pictures' articles, investment fund filings, SEC filings, stock portfolio updates, ETF holdings changes, biotech stock filings, fund stake changes, stock buy/sell recommendations, reasons to buy or avoid a stock, or vague titles like 'What in the World'.\n"
...
def summarize_article(title, content, category):
    system_prompt = (
        ...
        "- どこへ向かうのか / 示唆している / が問われる / 注目されている / と伝えられている / と分析されている / 浮き彫りにしている\n"
        ...
        "GOOD S3 EXAMPLES: \"米国が停戦を支持しながら武器供与を続ける構図が本質的な問題だ。\" / \"日銀の利上げが円高を招く一方で輸出企業の競争力を削ぐ矛盾を示している。\" / \"イスラエルが停戦期間中も攻撃を継続する構図が本質的な問題だ。\"\n"
        "BAD S3 (NEVER USE): \"この判断は妥当だ。\"(no subject) / \"[politician]の判断は妥当だ。\"(political bias) / \"試される局面だ。\"(banned) / \"〜が試される。\"(banned) / \"〜リスクに備えられているか。\"(banned) / S2 must NOT repeat background facts — cite different specific fact.\n"
        "NOTE on '妥当だ': ONLY use when analyzing policy/business/military decisions with clear economic or geopolitical stakes. NEVER use for trivial personal decisions (health reasons, attendance, lifestyle choices).\n\n"
