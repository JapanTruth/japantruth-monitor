...
def summarize_article(title, content, category):
    system_prompt = (
        ...
        "Convert English news articles into high-quality Japanese articles as an expert in politics, economics, and international affairs.\n\n"
        "ABSOLUTE RULES:\n"
        ...
        "- Background section: use only facts stated in the article plus basic general knowledge. No invented details.\n"
        "- If information is insufficient, reduce sentence count. Never fabricate to fill space.\n\n"
        "STRICTLY FORBIDDEN PHRASES (never use these):\n"
        "- 見守る / 注視する / 検討する / 可能性がある / かもしれない\n"
        "- 〜の重要性を示す / 注目が集まる / 求められる / 懸念される / 期待が高まる\n"
        "- Any vague filler sentence that adds no factual value.\n\n"
        "JAPANTRUTH PERSPECTIVE RULES:\n"
        "- If clear Japan impact exists: analyze specific effects on Japan's trade, energy, diplomacy, security, or financial markets using facts and numbers.\n"
        "- If Japan connection is weak: analyze effects on global economy, geopolitics, international order, or markets concretely. Never force a Japan connection.\n"
        "- Required expressions: 〜が予想される / 〜の公算が大きい / 〜と見られる / 〜は避けられない / 〜に直結する\n"
        "- Write 3-4 sentences, specific and assertive. No abstract conclusions.\n\n"
    ...
    prompt = (
        ...
        "  ## JapanTruthの視点\\n"
        "  (3-4 sentences. Be SPECIFIC and ASSERTIVE. CRITICAL: Do NOT force a Japan connection if none exists naturally. If clear Japan impact exists (energy, trade, security, yen): analyze with concrete numbers or facts. If NO Japan link: analyze effects on global markets, geopolitics, or international institutions ONLY - never mention Japan. MANDATORY: end with 〜が予想される or 〜は避けられない or 〜の公算が大きい. ABSOLUTELY FORBIDDEN: 可能性がある/見守る/注視する/求められる/懸念される/かもしれない.)\n"
    )
