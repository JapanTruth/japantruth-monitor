...
def screen_article(title, summary="", recent_titles=None):
    ...
    prompt = (
        ...
        "2. Is this topic already covered in the recently covered articles above? Answer yes if the SAME EVENT or SAME INCIDENT is already reported, even if described differently. Answer no if: clearly different event, different country, or different person.\n"
        "   SAME=YES: shooting at San Diego mosque + killing at San Diego Islamic Center. SAME=YES: Putin ceasefire + Putin announces ceasefire. DIFFERENT=NO: Trump tariffs EU + Trump Iran deal.\n"
...
def summarize_article(title, content, category):
    system_prompt = (
        ...
        "ROLES: Senate=上院 / House=下院 / Vice President=副大統領 / Prime Minister=首相 / Chancellor=首相 / Speaker=議長 / Chief Justice=最高裁長官 / Pentagon=国防総省 / Treasury=財務省 / Fed=連邦準備制度 / Secretary of State=国務長官 / Attorney General=司法長官\n"
    ...
    prompt = (
        ...
        "JSON fields:\n\n"
        "title: Active voice. State the OUTCOME or IMPACT. Include number/name OR contrast/paradox. If no number: use contrast between stated goal and actual result. "
        ...
        "## 背景\n"
        "2-4 sentences. Context that makes the news meaningful. NEVER repeat 何が起きているのか. Each sentence adds NEW info — history, previous policy, scale.\n\n"
        "## JapanTruthの視点\n"
        "EXACTLY 3 sentences:\n"
        "S1: Open with SPECIFIC NUMBER or NAMED ENTITY. Never: この/その/同社/同国.\n"
        "S2: MUST contain a specific year, number, or proper name NOT already mentioned. "
        ...
        post_data = {
            ...
            "record": {
                ...
                "facets": (lambda t_enc: [
                    ...
                    {
                        "index": {"byteStart": t_enc.index(tag.encode()), "byteEnd": t_enc.index(tag.encode()) + len(tag.encode())},
                        "features": [{"$type": "app.bsky.richtext.facet#tag", "tag": tag[1:]}]
                    }
                    for tag in hashtags.split()
                    if tag.startswith("#") and tag.encode() in t_enc
                ])(text.encode())
            }
        }
...
def score_article(result):
    ...
    # 禁止ワードチェック
    forbidden = ["かもしれない", "とみられる", "とされる", "示唆している", "と見られる", "試される局面だ", "深刻な局面だ", "どこへ向かうのか", "避けられない", "この判断は妥当だ", "妥当だと考えられる"]
