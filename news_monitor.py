...
def summarize_article(title, content, category):
    system_prompt = (
        ...
        "CRITICAL PROPER NOUN TRANSLATIONS:\n"
        "- Elon Musk=イーロン・マスク（エロンは誤り）\n"
        "- Modi=ナレンドラ・モディ, BJP=インド人民党（BJP）, TMC=全インド草の根会議派（TMC）\n"
        "- Fed=連邦準備制度（FRB）, Berkshire=バークシャー, Hantavirus=ハンタウイルス\n"
        "- Mark Hamill=マーク・ハミル, Hakeem Jeffries=ハキーム・ジェフリーズ\n"
        "- Political Commissar=政治委員（政治委員長は誤り）\n"
        "- Pezeshkian=ペゼシュキアン（ペゼーシキアンは誤り）\n"
        "- Mogami=もがみ型（固有名詞。最上級と訳すな）\n\n"
        ...
        "- Use ONLY facts explicitly stated in the source article.\n"
        "- Exception: universally known facts (e.g. country capitals, well-known historical events) are allowed.\n"
        "- NEVER add statistics, percentages, names, dates, or figures not present in the source.\n"
        "- If background cannot be written without adding unverified facts: omit entirely.\n\n"
        ...
        "Sentence 3 — SHARP CONCLUSION:\n"
        "  Choose ONE: 〜が予想される / 〜と見られる / 〜は避けられない状況だ / question ending 〜はどう動くべきか / 〜はこのリスクに備えられているか / 〜はどこへ向かうのか\n"
        "  Never end with abstract philosophical statement. Never repeat sentence 1 or 2.\n\n"
    ...
    prompt = (
        ...
        "- excerpt: One Japanese sentence summarizing who did what and why. MUST differ from the first sentence of the body. Do not copy the opening line of 何が起きているのか.\n"
