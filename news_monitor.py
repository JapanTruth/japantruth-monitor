...
def screen_article(title, summary="", recent_titles=None):
    ...
    prompt = (
        ...
        "1. Is this newsworthy? Answer yes if: war/conflict, diplomacy/summit, economic news, politics, business/corporate news, crime, social issues, sports, science/tech, environment. Answer no if: clearly trivial, opinion pieces, roundups/summaries of already-known events, 'what to watch' preview articles, political cartoons/satire compilations, weekly photo galleries, 'best of' collections, news quizzes, photo essays, 'in pictures' articles, investment fund filings, SEC filings, stock portfolio updates, ETF holdings changes, biotech stock filings, fund stake changes, stock buy/sell recommendations, reasons to buy or avoid a stock, reading lists, gift guides, best-of lists, lifestyle tips for wealthy, summer reading, celebrity/politician lifestyle stories, building renovation/construction plans, reading lists for wealthy, personal moving/relocation stories, individual life change stories, personal finance tips, or vague titles like 'What in the World'.\n"
...
def summarize_article(title, content, category):
    system_prompt = (
        ...
        "STRICTLY FORBIDDEN (speculation/bias only):\n"
        "- SPECULATION: とみられる/とされる/示唆している/かもしれない/と見られる/と伝えられている\n"
        "- POLITICAL BIAS: [country/leader]の判断は妥当だ/避けられない\n"
        "- VAGUE ENDINGS: 試される局面だ/深刻な局面だ/どこへ向かうのか\n\n"
        "STYLE GUIDANCE (avoid overuse):\n"
        "- Prefer specific facts over: 高まっている/広がっている/注目されている/見通しだ\n"
        "- S3: concrete subject required, not この/その/同社/同国\n\n"
        ...
        "NEUTRALITY: Never endorse/condemn governments or leaders. No editorial opinions (迅速/適切/重要なステップ). 背景 must NOT repeat 何が起きているのか.\n\n"
    ...
    prompt = (
        ...
        "  Start with number/name/surprising verb. NEVER copy 何が起きているのか first sentence.\n"
        "- keyword: 1-3 English words for Unsplash image search. Must be a concrete visual subject.\n"
        "  Use concrete visual nouns (city name, object). Avoid abstract words.\n"
        "- category: politics/economy/international/investment/culture (see system prompt for definitions)\n"
